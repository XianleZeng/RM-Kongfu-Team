...
float Now_Omega_Wheel_0, Target_Omega_Wheel_0;
float Now_Omega_Wheel_1, Target_Omega_Wheel_1;
float Now_Omega_Wheel_2, Target_Omega_Wheel_2;
float Now_Omega_Wheel_3, Target_Omega_Wheel_3;
...
int32_t Output_val_wheel_0, Output_val_wheel_1, Output_val_wheel_2, Output_val_wheel_3;

float test_val = 0;
float Right_X, Right_Y, Left_X, Left_Y, Switch_0, Switch_1;
// 摇杆偏移量
...
//地盘最大速度
float velocity_x_max = 2;
float velocity_y_max = 2;
float omega_max = 0.005;
...
float Rocker_Yaw_Resolution = 0.001f * PI;
...
float GEAR_RATE_Friction = 1.0f;
float GEAR_RATE_Wheel = 3591.0f / 187.0f;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    usart1_tx_dma_init();
		...
		Motor_Yaw.Init(&hcan2, Motor_DJI_Control_Method_ANGLE, 25000 ,16384);
    ...
    Motor_Friction_Right.Init(&hcan1, Motor_DJI_Control_Method_OMEGA, 0, 20);
		Motor_Wheel[0].Init(&hcan2, Motor_DJI_Control_Method_OMEGA, 0, 20);
		Motor_Wheel[1].Init(&hcan2, Motor_DJI_Control_Method_OMEGA, 0, 20);
		Motor_Wheel[2].Init(&hcan2, Motor_DJI_Control_Method_OMEGA, 0, 20);
		Motor_Wheel[3].Init(&hcan2, Motor_DJI_Control_Method_OMEGA, 0, 20);
		// PID初始化
    ...
    Motor_Friction_Left.PID_Omega.Init(0.15f, 0.0f, 0.000f, 0.0f, 10.0f, 10.0f);
    Motor_Friction_Right.PID_Omega.Init(0.15f, 0.0f, 0.000f, 0.0f, 10.0f, 10.0f);
		Motor_Wheel[0].PID_Omega.Init(0.15f, 0.0f, 0.000f, 0.0f, 10.0f, 10.0f);
		Motor_Wheel[1].PID_Omega.Init(0.15f, 0.0f, 0.000f, 0.0f, 10.0f, 10.0f);
		Motor_Wheel[2].PID_Omega.Init(0.15f, 0.0f, 0.000f, 0.0f, 10.0f, 10.0f);
		Motor_Wheel[3].PID_Omega.Init(0.15f, 0.0f, 0.000f, 0.0f, 10.0f, 10.0f);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		/*从遥控器读取数据*/
		Right_X = (local_rc_ctrl->rc.ch[0] -  Rocker_Offset - 17) / Rocker_Num; /* 右侧左右 */
		Right_Y = (local_rc_ctrl->rc.ch[1] -  Rocker_Offset - 13) / Rocker_Num; /* 右侧上下 */
		Left_Y = (local_rc_ctrl->rc.ch[2] -  Rocker_Offset) / Rocker_Num; /* 左侧上下 */
		Left_X = (local_rc_ctrl->rc.ch[3] -  Rocker_Offset) / Rocker_Num; /* 左侧左右 */
    Switch_1 = local_rc_ctrl->rc.s[1];// - Switch_Offset); // / Switch_Num;
		Switch_0 = (local_rc_ctrl->rc.ch[4] - Switch_Offset) / Switch_Num;

		vx_set = Right_X*velocity_x_max;
		vy_set = Right_Y*velocity_y_max;
		omega_set = Left_X*omega_max;
		
		Target_Omega_Wheel_0 = (-0.707f * vx_set + 0.707f * vy_set + omega_set*r_wheel_to_core) / 0.075f;
		Target_Omega_Wheel_1 = (-0.707f * vx_set - 0.707f * vy_set + omega_set*r_wheel_to_core) / 0.075f;
		Target_Omega_Wheel_2 = ( 0.707f * vx_set - 0.707f * vy_set + omega_set*r_wheel_to_core) / 0.075f;
		Target_Omega_Wheel_3 = ( 0.707f * vx_set + 0.707f * vy_set + omega_set*r_wheel_to_core) / 0.075f;
...
//		Motor_Yaw.Target_Omega = Target_Omega_0;
		...
		Target_Yaw_Angle = 0; // Left_X*Rocker_Yaw_Resolution;
    ...
    Motor_Friction_Right.Set_Target_Omega(Friction_Omega);
		...
		Motor_Wheel[0].Set_Target_Omega(Target_Omega_Wheel_0);
		Motor_Wheel[1].Set_Target_Omega(Target_Omega_Wheel_1);
		Motor_Wheel[2].Set_Target_Omega(Target_Omega_Wheel_2);
		Motor_Wheel[3].Set_Target_Omega(Target_Omega_Wheel_3);
    ...
    frictoin_motor_measure_left = get_friction_motor_measure_point(0);
    frictoin_motor_measure_right = get_friction_motor_measure_point(1);
    ...
    /*写入电机数据*/
		...
		Motor_Driver.Rx_Data.Now_Omega = Motor_Driver_measure->speed_rpm * RPM_TO_RAD / GEAR_RATE;
    ...
    Motor_Friction_Right.Rx_Data.Now_Omega = frictoin_motor_measure_right->speed_rpm * RPM_TO_RAD / GEAR_RATE_Friction;
		...
		Motor_Wheel[0].Rx_Data.Now_Omega = Rx_Omega[0] * RPM_TO_RAD / GEAR_RATE_Wheel;
		Motor_Wheel[1].Rx_Data.Now_Omega = Rx_Omega[1] * RPM_TO_RAD / GEAR_RATE_Wheel;
		Motor_Wheel[2].Rx_Data.Now_Omega = Rx_Omega[2] * RPM_TO_RAD / GEAR_RATE_Wheel;
		Motor_Wheel[3].Rx_Data.Now_Omega = Rx_Omega[3] * RPM_TO_RAD / GEAR_RATE_Wheel;
  ...
  Motor_Friction_Right.PID_Calculate();
	Motor_Wheel[0].PID_Calculate();
	Motor_Wheel[1].PID_Calculate();
	Motor_Wheel[2].PID_Calculate();
	Motor_Wheel[3].PID_Calculate();
	...
	Output_val_wheel_0 = Motor_Wheel[0].Out;
	Output_val_wheel_1 = Motor_Wheel[1].Out;
	Output_val_wheel_2 = Motor_Wheel[2].Out;
	Output_val_wheel_3 = Motor_Wheel[3].Out;
		
	/*画出目标角度和电机转速*/	
	serialplot.Set_Data(3, &Right_X, &Right_Y, &Left_X);
	...
	if (Switch_0 == 1)
	{
		CAN_cmd_chassis(Output_val_wheel_0, Output_val_wheel_1, Output_val_wheel_2, Output_val_wheel_3);
		if (Switch_1 == 0)
		{
			CAN_cmd_friction(Output_val_friction_left, Output_val_friction_right, 0, 0);
	//		CAN_cmd_gimbal(Output_val_yaw, 0, 0, 0);
      CAN_cmd_shoot(0, Output_val_pitch, 0, 0);
		}
		else if (Switch_1 == 15)
		{
			CAN_cmd_friction(Output_val_friction_left, Output_val_friction_right, 0, 0);
			CAN_cmd_shoot(0, Output_val_pitch, Output_val_driver, 0);
	//			CAN_cmd_gimbal(Output_val_yaw, 0, 0, 0);
		}
		else if (Switch_1 == 240)
		{
