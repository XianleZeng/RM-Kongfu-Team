...
uint32_t Counter = 0;
int32_t Output_val_pitch, Output_val_driver, Output_val_2,Output_val_3;
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    Motor_Friction_Left.PID_Omega.Init(0.1f, 0.3f, 0.002f, 0.0f, 10.0f, 10.0f);
    Motor_Friction_Right.PID_Omega.Init(0.1f, 0.3f, 0.002f, 0.0f, 10.0f, 10.0f);
...
//		Motor_Yaw.Target_Omega = Target_Omega_0;
    ...
    Motor_Friction_Right.Set_Target_Omega(Friction_Omega);
			...
			/*读取电机数据*/
		gimbal_motor_measure = get_pitch_gimbal_motor_measure_point();
    ...
    Now_Omega_1 = frictoin_motor_measure_left->speed_rpm * RPM_TO_RAD / GEAR_RATE_Friction;
   ...
   else if (delta_encoder > Encoder_Num_Per_Round / 2)
   {
	...
	Motor_Pitch.Rx_Data.Total_Encoder = Motor_Pitch.Rx_Data.Total_Round * Encoder_Num_Per_Round + gimbal_motor_measure->ecd + Encoder_Offset;
  ...
  Motor_Friction_Right.PID_Calculate();
	...
	/*画出目标角度和电机转速*/	
	//serialplot.Set_Data(3, &Motor_Pitch.Rx_Data.Now_Angle, &Motor_Pitch.Target_Angle, &Output_val_pitch);
	serialplot.Set_Data(3, &Now_Omega_1, &Motor_Friction_Right.Rx_Data.Now_Omega, &Motor_Driver.Rx_Data.Now_Omega);
